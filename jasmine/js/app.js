...
const Jasmine = {
    ...
    */
    ...
    /*
    Trip dates are stored as plain "YYYY-MM-DD" strings. new Date("YYYY-MM-DD")
    parses that as UTC midnight, but "today" (elsewhere) is computed in the
    browser's local time. In timezones ahead of UTC (e.g. Nairobi, UTC+3) that
    mismatch made a trip departing "today" still compare as Upcoming until
    3am local time the next day. Parsing manually as a local date keeps both
    sides of every comparison in the same timezone.
    */
    ...
    formatDateRange(departure, ret){
        ...
        const start = this.parseLocalDate(departure);
        ...
        const end = this.parseLocalDate(ret);
    ...
    getTripStatus(trip){
        ...
        const start = this.parseLocalDate(trip.departure);

        const end = trip.return ? this.parseLocalDate(trip.return) : start;
