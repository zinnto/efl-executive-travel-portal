...
async function renderPolicy() {
  ...
  try {
    ...
    wrap.innerHTML = `
      ...
      <p class="policy-note" style="margin:-8px 2px 12px;">"Fill for This Trip" generates a copy pre-filled from ${TRIP.traveller.name}'s saved trip details — review before submitting.</p>
      <div class="info-card" style="padding:8px 12px;">
        ${(p.forms || []).map(f => `
          <div class="doc-row" style="flex-wrap:wrap;">
            <div class="doc-row__icon">${ICONS.pdf}</div>
            <div class="doc-row__name" style="flex:1 1 140px;">${f.name}<div style="font-size:10.5px;color:var(--ink-300);font-weight:600;text-transform:uppercase;letter-spacing:.04em;margin-top:2px;">${f.timing || ''}</div></div>
            <a class="btn btn-outline btn-sm" href="${f.file}" target="_blank" rel="noopener" style="flex:none;">Blank</a>
            <button class="btn btn-primary btn-sm" data-generate-form="${f.key}" style="flex:none;">${ICONS.document} Fill for This Trip</button>
          </div>`).join('')}
      </div>
    ...
    wrap.querySelectorAll('[data-generate-form]').forEach(btn => {
      btn.addEventListener('click', () => handleGenerateForm(btn.dataset.generateForm, btn));
    });
  } catch (err) {
...
/* ---------------- Generate filled policy forms (client-side PDF) ---------------- */
...
async function generateMealPerDiem(trip) {
  ...
  const policyRow = (POLICY_CACHE && trip.traveller.zone) ? POLICY_CACHE.perDiemByZone.rows.find(r => r.zone === trip.traveller.zone) : null;
...
async function handleGenerateForm(kind, btn) {
  ...
  try {
    if (kind === 'trf') await generateTRF(TRIP);
    else if (kind === 'perdiem') await generateMealPerDiem(TRIP);
    else if (kind === 'claim') await generateExpenseClaim(TRIP);
    else if (kind === 'posttravel') await generatePostTravelReport(TRIP);
    else { showToast('Unknown form type.'); return; }
    showToast('Form generated — check your downloads.');
  } catch (err) {
