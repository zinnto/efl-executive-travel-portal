...
/* ---------------- Generate filled policy forms (client-side PDF) ----------------
   The actual generation logic lives in forms.js (shared with the admin
   dashboard) — this is just the portal's UI wiring around it. */
...
async function handleGenerateForm(kind, btn) {
  ...
  try {
    await generateFormByKind(kind, TRIP, POLICY_CACHE);
